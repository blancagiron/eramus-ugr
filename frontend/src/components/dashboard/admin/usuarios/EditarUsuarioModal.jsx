...
//       });
//     fetch("http://localhost:5000/api/destinos/ocupacion")
//       .then((r) => r.json())
//       .then(setOcupacion)
//       .catch(() => setOcupacion({}));
//   }, []);

//   const plazasDisponiblesDe = (destino) => {
//     const total = destino.plazas ?? 0;
//     const ya = Number(ocupacion?.[destino.codigo] ?? 0);
//     const yaAsignadoEste = (usuario.destinos_asignados || []).some(d => d.codigo === destino.codigo);
//     const ocupadosEfectivos = Math.max(0, ya - (yaAsignadoEste ? 1 : 0));
//     return Math.max(0, total - ocupadosEfectivos);
//   };
...
//         if (destino) {
//           const occ = await fetch(`http://localhost:5000/api/destinos/${encodeURIComponent(destino.codigo)}/ocupacion`).then(r => r.json());
//           const total = Number(todosLosDestinos.find(d => d.codigo === destino.codigo)?.plazas ?? 0);
//           const ya = Number(occ?.ocupados ?? 0);
//           const yaAsignadoEste = inicial?.rol === "estudiante" && (inicial.destinos_asignados || []).some(d => d.codigo === destino.codigo);
//           const ocupadosEfectivos = Math.max(0, ya - (yaAsignadoEste ? 1 : 0));
//           if (ocupadosEfectivos >= total) {
//             setMensaje("No quedan plazas disponibles para ese destino.");
//             return;
//           }
...
//                           const isAsignado = (usuario.destinos_asignados || []).some((d) => d.codigo === destino.codigo);
//                           const plazasDisponibles = plazasDisponiblesDe(destino);
//                           const sinPlazas = plazasDisponibles <= 0 && !isAsignado;
...
//                                   {destino.codigo} — Plazas: {plazasDisponibles}/{destino.plazas ?? 0}
//                                 </div>
//                               </div>
//                               {!sinPlazas && (
...
// 
import { useState, useEffect } from "react";
import { X, Search, Plus, Trash2, MapPin, Building, User, GraduationCap } from "lucide-react";
...
export default function EditarUsuarioModal({ usuario: inicial, onClose, esNuevo = false }) {
  ...
  // ---- plazas disponibles (contempla si ya tenía ese destino) ----
  const plazasDisponiblesDe = (destino) => {
    const total = destino.plazas ?? 0;
    const ya = Number(ocupacion?.[destino.codigo] ?? 0);
    const yaAsignadoEste = (usuario.destinos_asignados || []).some((d) => d.codigo === destino.codigo);
  ...
  // ---- filtros de búsqueda generales (texto) ----
  const destinosFiltrados = todosLosDestinos.filter((d) =>
    `${d.nombre_uni} ${d.codigo}`.toLowerCase().includes(busquedaDestino.toLowerCase())
  );

  // ---- SOLO PARA TUTORES (tu lógica original de disponibles) ----
  const destinosDisponibles = todosLosDestinos.filter(
    (d) =>
      (!d.tutor_asignado || d.tutor_asignado === usuario.email) &&
      d.codigo_centro_ugr === usuario.codigo_centro
  );

  const destinosDisponiblesFiltrados = destinosDisponibles.filter(
    (d) =>
      `${d.nombre_uni} ${d.codigo}`.toLowerCase().includes(busquedaDestino.toLowerCase()) &&
      !(usuario.destinos_asignados || []).some((asignado) => asignado.codigo === d.codigo)
  );

  // ---- RESTRICCIÓN POR GRADO PARA ESTUDIANTES ----
  const esDestinoDelGrado = (d) =>
    String(d?.codigo_grado_ugr ?? "") === String(usuario?.codigo_grado ?? "");

  const destinosDeSuGrado = todosLosDestinos.filter(esDestinoDelGrado);

  const destinosFiltradosEstudiante = destinosDeSuGrado.filter((d) =>
    `${d.nombre_uni} ${d.codigo}`.toLowerCase().includes(busquedaDestino.toLowerCase())
  );
  ...
  const agregarDestino = (destino) => {
    // BLOQUEO: solo destinos del mismo grado para estudiantes
    if (usuario.rol === "estudiante" && !esDestinoDelGrado(destino)) {
      setMensaje("Solo puedes asignar destinos del mismo grado del estudiante.");
      return;
    }
  ...
  const handleGuardar = async () => {
    try {
      ...
      if (usuario.rol === "estudiante") {
        const destino = usuario.destinos_asignados?.[0];
        if (destino) {
          // Revalidación de GRADO
          const destCompleto = todosLosDestinos.find((d) => d.codigo === destino.codigo);
          if (!esDestinoDelGrado(destCompleto)) {
            setMensaje("El destino seleccionado no pertenece al grado del estudiante.");
            return;
          }

          // Revalidación de PLAZAS
          const occ = await fetch(
            `http://localhost:5000/api/destinos/${encodeURIComponent(destino.codigo)}/ocupacion`
          ).then((r) => r.json());

          const total = Number(todosLosDestinos.find((d) => d.codigo === destino.codigo)?.plazas ?? 0);
          const ya = Number(occ?.ocupados ?? 0);
          const yaAsignadoEste =
            inicial?.rol === "estudiante" && (inicial.destinos_asignados || []).some((d) => d.codigo === destino.codigo);
          const ocupadosEfectivos = Math.max(0, ya - (yaAsignadoEste ? 1 : 0));
              ...
              <div>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    {/* Lista de destinos (SOLO los del grado del estudiante) */}
                    {mostrarListaDestinos && usuario.codigo_grado && destinosFiltradosEstudiante.length > 0 && (
                      <div className="mt-2 bg-white border border-gray-200 rounded-lg shadow-lg max-h-48 overflow-y-auto">
                        {destinosFiltradosEstudiante.slice(0, 10).map((destino) => {
                          const isAsignado = (usuario.destinos_asignados || []).some((d) => d.codigo === destino.codigo);
                          const plazasDisponibles = plazasDisponiblesDe(destino);
                          const sinPlazas = plazasDisponibles <= 0 && !isAsignado;
                          ...
                          return (
                            <button
                              key={destino.codigo}
                              onClick={() => !sinPlazas && agregarDestino(destino)}
                              className={`w-full text-left px-4 py-3 border-b border-gray-100 last:border-b-0 transition-colors flex items-center justify-between group ${
                                sinPlazas && !isAsignado ? "opacity-50 cursor-not-allowed" : "hover:bg-red-50"
                              }`}
                              disabled={sinPlazas && !isAsignado}
                              title={sinPlazas && !isAsignado ? "No hay plazas disponibles" : ""}
                            >
                    ...
                    {/* Aviso si no hay grado aún */}
                    {mostrarListaDestinos && !usuario.codigo_grado && (
                      <div className="mt-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">
                        Selecciona primero el grado del estudiante para ver destinos disponibles.
                      </div>
                    )}
